...
| Name | `layer.name` | When `undefined` or `""`, held as `null`. The layer panel supplies the placeholder |
...
| `name` is `undefined` or `""` | Held as `null`, and the layer panel displays its placeholder. The format always stores a name, so a layer created without one arrives as `""` rather than `undefined` |
