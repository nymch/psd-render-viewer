...
- ローカルのPSDを開くと、全レイヤーがPhotoshopと同じ重ね順でCanvasに描画される
...
1. **ファイルを開く** — `<input type="file">`で選ぶか、Canvas領域へドラッグ&ドロップする。開くとパース・合成・描画が走り、レイヤーパネルが埋まる。**読み込み中は`<input>`とドロップの受け付けを無効にする**（後述）
...
| パース結果（`PixelData`を含む生のレイヤー） | `usePsdDocument`内の`useRef` | 巨大なピクセルデータをReact stateに入れない |
| 合成結果の`ImageBitmap`・中間の`OffscreenCanvas` | 同上 | cleanupで`close()`する |
...
**パースする側と描く側を同じコンポーネントに寄せる。**`DropZone`は受け取った`File`をatomへ書くだけにし、パース・合成・描画は`CanvasViewport`が`usePsdDocument`（`hooks/psdHooks.ts`（未作成））を通して行う。refはコンポーネント単位なので、`DropZone`でパースして`CanvasViewport`で描く形にするとピクセルの受け渡し先が無くなる。共通の親に置こうとすると`app/page.tsx`が`"use client"`になり、[react.md](../../.claude/rules/react.md)の「境界を末端へ押し下げる」と衝突する。
...
hooks/
└─ psdHooks.ts   ← usePsdDocument。パース結果と中間バッファのrefを持つ
...
DropZone           読み込みの試みを{status:"parsing", file}にする
 → CanvasViewport   usePsdDocumentがそれを見て以下を順に呼ぶ
   → 1フレーム譲る   ローディング表示を塗らせる
   → parse.ts       file.arrayBuffer() → readPsd（メインスレッド・同期）
   → limits.ts      長辺と面積のチェック。超えたらここで止める
   → tree.ts        LayerNode[]をatoms/layers.tsへ + ピクセルの表をrefへ
   → composite.ts   OffscreenCanvasへ再帰合成 → ImageBitmap
   → drawImageで1回描く
...
**`readPsd`は同期関数なので、状態を`parsing`にした直後に呼ぶとローディング表示が一度も塗られない。**状態をセットしてからパースを終えるまでが1つのタスクの中で完結し、その間ブラウザは描画できないまま表示中のドキュメントの差し替えまで進む。

パースの前に1フレーム譲る。`requestAnimationFrame`を2回待ってから`readPsd`を呼ぶ。1回目のコールバックはそのフレームの描画前に走るため、実際に塗られたことを保証するには2回目まで待つ必要がある。
...
これは[資源の解放](#資源の解放)で「入れない」としたフレーム集約とは別物。あちらは毎フレーム値が変わる操作で描画がキューに積まれるのを防ぐもので、こちらは1回だけ描画の機会を作るためのもの。

**この待ちの間と`file.arrayBuffer()`の`await`の間は、ユーザーの操作を受け付けられてしまう。**2つ目のファイルを落とすとパースが二重に走るため、読み込み中は`DropZone`と`<input type="file">`を無効にする。同期実行中のパース本体は元々操作を受け付けないので、無効にする必要があるのはこの2箇所の`await`の間だけ。
...
面積の上限はメモリから逆算する。ドキュメントサイズで確保されるのは表示用Canvas・ルートのバッファ・合成結果の`ImageBitmap`の3枚。
...
`composite.ts`はDOMに触れず`OffscreenCanvas`と`ImageData`だけで完結させる。後からWeb Workerへ移すときにインターフェースを変えずに済む。

### 資源の解放

`ImageBitmap`と`OffscreenCanvas`はcleanupで解放する（`bitmap.close()`）。新しいファイルを開いたときは前のファイルの資源を先に解放する。PSDは1枚でも数百MBのピクセルデータになるため、解放漏れがそのままメモリ枯渇につながる。

合成中の中間バッファは、親へ`drawImage`し終えた時点でその場で解放する。cleanupまで持ち越さない。

このバージョンは再描画のきっかけが「ファイルを開く」しか無いので、`requestAnimationFrame`による**フレーム集約**は入れない。表示切替や不透明度スライダーを足すときに導入する。読み込み中の表示のために1フレーム譲る使い方は別物で、そちらは入れる。
...
- **最初からWeb Workerへ分離する** — 大きいPSDでUIが固まらない利点があるが、絵が違うときに「合成が間違っているのか転送で壊れたのか」の切り分けが増える。`OffscreenCanvas`はメインスレッドでも使えるので、`composite.ts`をDOM非依存に書いておけば移行コストは小さい
...
`test/fixtures/`へ検証用のPSDを置き、Photoshopの表示と並べて見比べる。何を確かめるためのファイルかは`test/fixtures/README.md`（未作成）に残す。

**比較の前に、Photoshop側でも未対応の要素をオフにする。**調整レイヤーは非表示にし、レイヤー効果はレイヤースタイルのチェックを外す。両方から同じものを抜いた状態で見比べるため、残った差分はすべて合成のバグと判定できる。
...
| `"pass through"`のグループ + グループの不透明度50%。中の2枚を重ねておく | 1 | 通過グループの不透明度の扱い。Photoshopが分離するなら上の行と同じ絵になり、そうでなければ重なり部分が濃くなる。未解決の論点の判定に使う |
