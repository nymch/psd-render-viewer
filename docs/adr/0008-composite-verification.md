...
**The reference from here is FireAlpaca.** The spec settled that at [psd-viewer-v1.md:329](../design/psd-viewer-v1.md) for the same reason and accepted the cost: a blend mode where FireAlpaca and Photoshop disagree is missed. The two render closer to each other than either does to GIMP or Paint.NET, which makes FireAlpaca a workable stand-in rather than merely the only thing left.

**Its reach is narrower than that framing suggests, and the limit is structural rather than a matter of degree.** FireAlpaca has ten blend modes — normal, multiply, add/glow, overlay, screen, lighten, darken, difference, dodge, burn — against the 27 a PSD can carry and the 17 this app maps. A file using `linear dodge`, `soft light`, `hard light`, or any non-separable mode is compared against a renderer that does not have it, so the difference is the reference's. **This app supports more blend modes than the renderer being used to check it.** Three of the first four files measured turned out to use modes FireAlpaca lacks.

So a second reference is needed for anything outside those ten, and one is available where an author distributes their own render alongside the file. That passes through no importer and no exporter, which makes it the only reference with nothing translated on the way — at the cost of usually being a JPEG.
...
`lib/psd/composite.ts` delegates blending to `globalCompositeOperation`, so **the blend math belongs to the browser, not to this repository.** A Node canvas is a different implementation of that math. When its output disagreed with FireAlpaca, nothing would say whether this code, the polyfill, or the mapping table was at fault — and the mapping table is the thing under test. It is rejected for failing criterion 1 outright, not for being slow or awkward.

### Regression-only is the cheapest thing that works, and does not answer the question

Comparing a render against this repository's own previous render needs no reference renderer, no export settings, and no tolerance — the measurement below puts its threshold at zero. It would catch any change in rendering, it can run unattended, and it sidesteps every open question about FireAlpaca.
...
| Reference | a lossless export from another renderer | to be calibrated | is the composite wrong |
| Lossy | an image the author published as the correct result | measured per tile, per run | is the composite wrong, where no exporter can be trusted |

**One script, not three.** Reading pixels out of the app, attributing a difference by layer, and formatting the report are identical across them; only where the baseline comes from differs, and how finely it can be compared. Splitting them would duplicate all of it to vary one argument.

By eye and regression-only both reach the cause, so criterion 1 does not separate them. **Criterion 2 is what settles it, and it does so unevenly enough to state plainly:** by eye cannot be re-run at all, regression re-runs exactly, and the reference modes sit between them — they depend on an image produced by hand, which is why the export format is pinned below rather than left to the day it is used. The modes that answer the question are the less repeatable half. Fixing that as far as it can be fixed means pinning everything about the export that is under anyone's control.
...
**For comparison against FireAlpaca no number can be set yet**, and inventing one here would be worse than recording its absence. Two independent renderers differ through anti-aliasing, color management, and premultiplied-alpha rounding, none of which has been measured because no Alpaca export has been obtained. The calibration procedure is the decidable part: **measure the noise floor on generated fixtures whose answer is known** — a plain three-layer stack should come out near zero — and set the tolerance from that floor, not from taste.

**For an author's distributed render it is measured per tile, per run.** Such an image is usually a JPEG, so a pixel comparison is meaningless — but a JPEG stores each 8x8 block's mean as its DC coefficient, the most finely quantised value in the table, so regional averages survive it well. Comparing by tile mean keeps what the format keeps.

The floor for that comparison need not be guessed either: **push our own render through JPEG at the same quality and read it back, and the round trip is what JPEG alone costs this picture.** Measured per tile, a detailed tile earns a high floor and a flat one a low floor, which no fixed threshold can do. A difference counts only once it clears its own tile's floor.

The asymmetry is the useful result. Regression detection is exact and can be automated now; a lossless reference comparison needs calibration and stays manual; a lossy one calibrates itself but answers at tile granularity rather than per pixel.
...
- Bad: **criterion 1 is not fully met for the files the discovery phase relies on.** Where a PSD was exported from another editor, a difference may belong to this app, to FireAlpaca, or to the export, and the report cannot say which. It narrows the search to a layer; it does not establish whose fault that layer is
...
- **Everything involving FireAlpaca.** No export has been obtained, so its color management, alpha handling, and export settings are unknown. If it applies an ICC profile the browser canvas does not, the noise floor could be large enough to swamp real differences, and the comparison would need color conversion before it means anything
...
- **That FireAlpaca is close enough to Photoshop to stand in for it.** The judgment is that the two are far nearer each other than either is to GIMP or Paint.NET, which is an assessment from use, not a measurement — and with Photoshop unavailable it cannot be turned into one here. Every finding this produces inherits it: a difference against FireAlpaca is evidence about Photoshop only as far as that closeness holds, and the modes where it is likeliest to fail are the ones ADR-0002 already marks unsupported
- **Whether FireAlpaca exports the same PSD to the same bytes twice.** The format above pins what a person chooses; it says nothing about whether the renderer behind it is deterministic. If it is not, the baseline moves between exports and the reference threshold has to absorb that as well. Two exports of one file, compared byte for byte, settle it, and that is worth doing before any expected image is stored
...
- **Whether an author's published render is a faithful reference.** It is the only one with no import or export step, but it reflects whatever rendered it, and for a file authored in Clip Studio Paint that is Clip Studio rather than Photoshop. Agreement with it says the picture matches what the author saw, which is not the same claim as matching Photoshop — and which of those two this app is actually for is the question deliberately left open above
